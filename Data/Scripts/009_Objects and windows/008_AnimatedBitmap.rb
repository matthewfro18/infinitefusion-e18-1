...
class AnimatedBitmap
  ...
  def pbSetColor(r = 0, g = 0, b = 0, a = 255)
    #dna = AnimatedBitmap.new("Graphics/Icons/iconDNA.png")
    for i in 0..@bitmap.bitmap.width
      for j in 0..@bitmap.bitmap.height
        #if dna.bitmap.get_pixel(i,j).alpha != 0
        #@bitmap.bitmap.set_pixel(i,j,dna.bitmap.get_pixel(i,j))
  ...
  def ttemmp
    color = Color.new(r, g, b, a)
    @bitmap.bitmap.fill_rect(@bitmap.bitmap.rect, color)
  end

  def getAvgColor(freq = 2)
    width = @bitmap.bitmap.width / freq
    height = @bitmap.bitmap.height / freq
    red = 0
    green = 0
    blue = 0
    n = width * height
    for x in 0...width
      for y in 0...height
        color = @bitmap.bitmap.get_pixel(x * freq, y * freq)
        if color.alpha > 0
          red += color.red
          green += color.green
          blue += color.blue
  ...
  def pbChangeHue(hue = 0)
    @bitmap.bitmap.hue_change(hue)
  end
end
